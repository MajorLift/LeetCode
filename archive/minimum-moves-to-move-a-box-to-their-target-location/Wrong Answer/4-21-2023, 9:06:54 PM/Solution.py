...
class Solution:
    def minPushBox(self, grid: List[List[str]]) -> int:
        ...
        # print(self.traversible((xs, ys), (xt, yt), (xb, yb)))
        ...
        while pq:
            ...
            for d in self.DIRECTIONS:
                ...
                is_, js = map(sum, zip((rb, cb), map(lambda x: -x, d)))
                if not self.valid(ib, jb) \
                    or not self.valid(is_, js) \
                    or not self.traversible((rs, cs), (is_, js), (xb, yb)):
                    continue
    ...
    def traversible(self, src, dst, box):
