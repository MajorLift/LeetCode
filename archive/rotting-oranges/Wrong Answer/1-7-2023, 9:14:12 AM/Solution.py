...
class Solution:
    ...
    def orangesRotting(self, init: List[List[int]]):
        ...
        self.init = init
        fresh = [(i, j) for i, j in product(range(self.m), range(self.n)) if init[i][j] == 1]
        if not fresh: 
            return 0
        rotten = [(i, j) for i, j in product(range(self.m), range(self.n)) if init[i][j] == 2]
        if not rotten:
            return -1
